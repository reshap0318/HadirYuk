...
graph TB
    ...
    subgraph Backend
        ...
        K --> S[Services]
        S --> T[Face Recognition Service (Optional)]
        S --> U[QR Code Service]
...
erDiagram
    ...
    USER_PROFILES {
        ...
        text face_embedding // OPTIONAL: only used if face recognition enabled
        datetime created_at
        datetime updated_at
    }
...
> **Note:** Photo field in check-in/check-out endpoints is for **evidence purposes only**. No face recognition matching is performed unless the optional face recognition feature is enabled.
