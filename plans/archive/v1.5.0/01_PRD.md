...
HadirYuk adalah sistem absensi karyawan kantor berbasis web yang modern, akurat, dan mudah digunakan. Sistem ini menggabungkan teknologi geotagging, photo evidence, dan QR code untuk memastikan keabsahan data kehadiran karyawan. Face recognition tersedia sebagai fitur opsional yang dapat ditambahkan di kemudian hari.

### Background

Banyak perusahaan masih menggunakan sistem absensi manual atau semi-digital yang rentan terhadap kecurangan seperti titip absen, manipulasi waktu, atau ketidakakuratan data kehadiran. HadirYuk hadir sebagai solusi digital yang mengintegrasikan validasi lokasi (geotagging), bukti foto saat check-in, dan kemudahan akses (QR code) dalam satu platform terpadu. Face recognition dapat ditambahkan sebagai lapisan validasi tambahan setelah sistem inti berjalan dengan baik.

### Specific Goals

- Mengurangi kecurangan absensi dengan validasi lokasi (geotagging) dan bukti foto saat check-in/out
...
| ------ | ----------- | ------------------------------------------------------------ | -------------------------------------- | ----------- |
| US-001 | Karyawan    | Saya bisa check-in menggunakan geotagging + foto sebagai bukti          | Absensi saya tervalidasi dengan akurat | Must Have   |
...
| Photo Evidence           | Upload/capture foto saat check-in/out sebagai bukti kehadiran      |
...
| Face Recognition | Validasi identitas otomatis saat check-in (memerlukan face photo) |
...
flowchart TD
    ...
    F -->|QR Code| H[Scan QR]
    G --> I[Capture/Upload Foto sebagai Bukti]
...
- Photo evidence disimpan dengan akses terkontrol
...
- Photo upload processing < 2 detik
...
- Face recognition menggunakan library open-source (opsional, tidak required untuk v1)
...
- Foto profil karyawan opsional (diperlukan hanya jika face recognition diaktifkan)
