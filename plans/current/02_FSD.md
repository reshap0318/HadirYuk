...
- **Business Logic:**
  ...
  - Frontend upload foto via `POST /api/upload` terlebih dahulu, mendapat `document_id` (UUID)
  - Frontend kirim `document_id` bersama latitude/longitude ke `POST /api/attendance/checkin`
  - Backend simpan record absensi dengan referensi foto dari `document_id`
  - Foto disimpan sebagai evidence (TIDAK dilakukan face recognition matching)
  - Catat: user_id, timestamp, latitude, longitude, method=geotagging, check_in_photo_url (dari document_id)
  ...
  - Jika `document_id` tidak valid, tampilkan error "Foto tidak valid. Silakan upload ulang"
...
- **Business Logic:**
  ...
  - Jika lokasi valid, user capture foto atau upload foto sebagai bukti kehadiran
  - Frontend upload foto via `POST /api/upload` terlebih dahulu, mendapat `document_id` (UUID)
  - Frontend kirim `document_id` bersama latitude/longitude ke `POST /api/attendance/checkout`
  - Backend update record absensi dengan referensi foto dari `document_id`
  - Foto disimpan sebagai evidence (TIDAK dilakukan face recognition matching)
  ...
  - Catat: check_out_time, check_out_lat, check_out_lng, check_out_method, check_out_photo_url (dari document_id), duration, status
...
| Submit Button   | Button | Enabled only if photo uploaded (document_id) |
...
flowchart TD
    ...
    L -->|"Ya"| N["Dapat document_id"]
    N --> O["Kirim document_id + lokasi ke POST /api/attendance/checkin"]
...
| document_id tidak valid            | "Foto tidak valid. Silakan upload ulang"                | Tidak simpan absensi            |
